...
def get_productos_service():
    ...
    cursor.execute("SELECT id, nombre, precio, tipo, alergenos, paradero, origen, stock, id_vendedor FROM producto")
...
def create_producto_service(producto_data):
    ...
    try:
        cursor.execute("""
            INSERT INTO producto (nombre, precio, tipo, alergenos, paradero, origen, stock, id_vendedor)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """, (
            producto_data['nombre'], producto_data.get('precio', 10.0), producto_data['tipo'], producto_data.get('alergenos'),
